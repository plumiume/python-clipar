...
from .basewrapper import AddArgumentOptions, ArgumentContainerProtocol, SubgroupWrapper, BoundWrapper
...
class LazyContainer(ArgumentContainerProtocol):
    ...
    arguments: list['LazyContainer._Argument']
    argument_groups: list['LazyContainer._ArgumentGroup']
...
class GroupWrapper(SubgroupWrapper[NS]):
    ...
    def __init__(
        ...
        self._parser_options = GroupWrapperOptions(
            ...
            description=namespace_type.__doc__,
        )
        self._parser_options.update(parser_options)
    ...
    def on_before_parse(
        self,
        bound_name: list[str],
        bound_wrapper: Union[BoundWrapper, None]
        ):
