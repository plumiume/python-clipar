...
# Python 3.11+ の新機能が必要な場合（利用可能な場合のみ）
from typing import TypeVarTuple, ParamSpec  # Python 3.11+
# または
from typing_extensions import TypeVarTuple, ParamSpec  # バックポート
...
# 共変性が必要な場合（型階層でサブタイプ関係を保持）
_T_COVARIANT = TypeVar('_T_COVARIANT', covariant=True)
_NS_COVARIANT = TypeVar('_NS_COVARIANT', covariant=True)
...
# bound制約 + 共変性の組み合わせ
_W_BOUND_COVARIANT = TypeVar('_W_BOUND_COVARIANT', bound='BaseWrapper[Any]', covariant=True)
```
...
### TypeVarの変性（Variance）
**必要に応じて `covariant=True` を追加**

#### 変性の種類
- **共変（Covariant）**: `covariant=True`
  - 型の階層関係を保持：`List[Child]` は `List[Parent]` のサブタイプになる
  - 使用場面：戻り値型、読み取り専用プロパティ
  - 例：`_T = TypeVar('_T', covariant=True)`

- **反変（Contravariant）**: `contravariant=True`
  - 型の階層関係を逆転：`Callable[[Parent], Any]` は `Callable[[Child], Any]` のサブタイプになる
  - 使用場面：関数の引数型
  - 例：`_T = TypeVar('_T', contravariant=True)`

- **不変（Invariant）**: デフォルト
  - 型の階層関係を保持しない：`List[Child]` と `List[Parent]` は無関係
  - 使用場面：読み書き両方が可能な場合

#### 実用例
```python
# 型階層でサブタイプ関係を維持する必要がある場合
_W = TypeVar('_W', bound='BaseWrapper[Any]', covariant=True)

# 複雑な継承階層でエラーが発生した場合の対処
# TypeError: Cannot inherit from Generic[...] multiple times with different parameters
# → TypeVarにcovariant=Trueを指定して解決
```

### Python 3.11+ の新機能対応
**Generic, TypeVar グループに TypeVarTuple, ParamSpec を追加**

#### TypeVarTuple（Python 3.11+）
```python
from typing import TypeVarTuple  # Python 3.11+
from typing_extensions import TypeVarTuple  # バックポート
...
#### ParamSpec（Python 3.11+）
```python
from typing import ParamSpec  # Python 3.11+
from typing_extensions import ParamSpec  # バックポート
...
#### 使用判断
- プロジェクトでPython 3.11+の機能を使用している場合のみ
- typing_extensionsでバックポート版を使用可能
- Python 3.10環境では基本的にTypeVar + Genericで十分
...
- [ ] **必要に応じて `covariant=True` が追加されている**（型階層のサブタイプ関係保持）
- [ ] Python 3.11+ の機能（TypeVarTuple, ParamSpec）が適切に対応されている
- [ ] インポートが正しく追加されている
- [ ] 構文エラーがない（仮想環境内で確認）
...
6. **共変性エラー**: 複雑な継承階層で型チェッカーがエラーを出す
   - `TypeError: Cannot inherit from Generic[...] multiple times with different parameters`
   - サブタイプ関係が正しく認識されない
...
7. **共変性の問題**: 必要に応じて `covariant=True` をTypeVarに追加
   - 型階層でサブタイプ関係を保持する必要がある場合
   - 継承チェーンで型パラメータが矛盾する場合
   - 例：`_W = TypeVar('_W', bound='BaseWrapper[Any]', covariant=True)`
