...
# 特定バージョンのユニットテスト
python -m pytest -q test/unit/v310    # Python 3.10/3.11 用
python -m pytest -q test/unit/v312    # Python 3.12+ 用

# 特定バージョンの統合テスト
python -m pytest -q test/integration/v310  # Python 3.10/3.11 用
python -m pytest -q test/integration/v312  # Python 3.12+ 用
...
### 各環境でのテスト実行例
```powershell
# Python 3.10環境でのテスト実行
venv\cp310\Scripts\activate
python -m pytest -q test/unit/v310 test/integration/v310 test/test_import_compatibility.py
deactivate

# Python 3.12環境でのテスト実行
venv\cp312\Scripts\activate
python -m pytest -q test/unit/v312 test/integration/v312 test/test_import_compatibility.py
deactivate
```

**注意**: 
- `venv/` ディレクトリは `.gitignore` で除外されているため、各開発者が個別に構築する
- 環境構築後は上記の「基本テスト実行」コマンドでテスト実行可能
...
python -m pytest -q test/unit/v312  # Python 3.12+ でのみ実行
